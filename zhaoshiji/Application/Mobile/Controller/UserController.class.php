...
class UserController extends MobileBaseController {
    ...
    public function express(){
    	...
    	$delivery = M('delivery_doc')->where(array('order_id'=>$order_id))->limit(1)->find();
    ...
    public function add_comment(){    
    	if(IS_POST){
    		...
    		if($_FILES['comment_img_file']['tmp_name'][0])
    		{
    			...
    			$upload->rootPath  =    UPLOAD_PATH.'comment/'; // 设置附件上传根目录
    			...
    			if(!$upinfo) {// 上传错误提示错误信息
    				...
    				{
    					$comment_img[] = C('IMG_URL').'Public/upload/comment/'.$val['savepath'].$val['savename'];
    				}
    ...
    */
    ...
    public function return_goods()
    {
	    ...
	    $spec_key = I('spec_key');
        $return_goods = M('return_goods')->where(array('order_id'=>$order_id,'goods_id'=>$goods_id,'spec_key'=>$spec_key))->find();
        ...
        if(IS_POST)
        {
    		...
    		if($_FILES['return_imgs']['tmp_name'][0])
    		{
    			...
    			$upload->maxSize   =    $map['author'] = (1024*1024*3);// 设置附件上传大小 管理员10M  否则 3M
    			$upload->exts      =    array('jpg', 'gif', 'png', 'jpeg');// 设置附件上传类型
    			$upload->rootPath  =    UPLOAD_PATH.'return_goods/'; // 设置附件上传根目录
    			$upload->replace   =    true; // 存在同名文件是否是覆盖，默认为false
    			//$upload->saveName  =  'file_'.$id; // 存在同名文件是否是覆盖，默认为false
    			// 上传文件
    			$upinfo  =  $upload->upload();
    			if(!$upinfo) {// 上传错误提示错误信息
    				...
    				foreach($upinfo as $key => $val)
    				{
    					$return_imgs[] = C('IMG_URL').'Public/upload/return_goods/'.$val['savepath'].$val['savename'];
    				}
