...
class ActivityController extends MobileBaseController {
    ...
    */ 
    public function group(){
        ...
        $group_buy_info = M('GroupBuy')->where("goods_id = $goods_id and NOW() >= start_time and NOW() <= end_time ")->find(); // 找出这个商品
    ...
    {
    	$count =  M('GroupBuy')->where("NOW() >= start_time and NOW() <= end_time ")->count();// 查询满足要求的总记录数
    	$Page = new \Think\Page($count,20);// 实例化分页类 传入总记录数和每页显示的记录数  	
    	$show = $Page->show();// 分页显示输出
    	$this->assign('page',$show);// 赋值分页输出
        $list = M('GroupBuy')->where("NOW() >= start_time and NOW() <= end_time ")->limit($Page->firstRow.','.$Page->listRows)->select(); // 找出这个商品
