...
function goods_thum_images($goods_id,$width,$height){
     ...
     if(empty($goods_id))
		 return '';
    //判断缩略图是否存在
    $path = C('IMG_URL')."Public/upload/goods/thumb/$goods_id/";
    ...
    $original_img = UPLOAD_PATH.substr($original_img,15); // 绝对路径
    ...
    $water = tpCache('water');
    if($water['is_mark']==1){
    	$imgresource = C('IMG_URL').$path.$goods_thumb_name;
...
function get_sub_images($sub_img,$goods_id,$width,$height){
    ...
    $original_img = UPLOAD_PATH.substr($sub_img['image_url'],15); // 绝对路径
	if(!file_exists($original_img)) return '';
...
function sendSMS($mobile, $code)
{
    ...
    {
        // 从数据库中查询是否有验证码
        $data = M('sms_log')->where("code = '$code' and add_time > ".date('Y-m-d H:i:s',(time() - 60*60)))->find();
        // 没有就插入验证码,供验证用
        empty($data) && M('sms_log')->add(array('mobile' => $mobile, 'code' => $code, 'add_time' => date('Y-m-d H:i:s'), 'session_id' => SESSION_ID));
        return true;        
    }
    else 
    {
...
function update_pay_status($order_sn,$pay_status = 1)
{
	if(stripos($order_sn,'recharge_') !== false){
		...
		M('recharge')->where("order_sn = '$order_sn'")->save(array('pay_status'=>1,'pay_time'=>date('Y-m-d H:i:s')));
		...
		// 修改支付状态  已支付
		M('order')->where("order_sn = '$order_sn'")->save(array('pay_status'=>1,'pay_time'=>date('Y-m-d H:i:s')));
		// 减少对应商品的库存
		...
		//分销设置
		M('rebate_log')->where(array('order_id'=>$order['order_id']))->save(array('status'=>1));
		// 成为分销商条件
    ...
    function confirm_order($id){
        ...
        $data['confirm_time'] = date('Y-m-d H:i:s'); // 收货确认时间
        if($order['pay_code'] == 'cod'){
        ...
        //分销设置
        M('rebate_log')->where(array('order_id'=>$id))->save(array('status'=>2,'confirm'=>date('Y-m-d H:i:s')));
...
{
	$order_goods = M('order_goods')->where(array('order_id'=>$order['order_id']))->cache(true)->select();
	//查找购买商品送优惠券活动
		...
		if($val['prom_type'] == 3)
        {
			...
			if($prom){
				...
				if($coupon && $coupon['createnum']>0){					                                        
                    ...
                    if($remain > 0)                                            
                    {
                        $data = array('cid'=>$coupon['id'],'type'=>$coupon['type'],'uid'=>$order['user_id'],'send_time'=>date('Y-m-d H:i:s'));
    ...
    $points = M('order_goods')->where(array('order_id'=>$order['order_id']))->sum("give_integral * goods_num");
