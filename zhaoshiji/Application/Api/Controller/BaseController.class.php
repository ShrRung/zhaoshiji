...
class BaseController extends Controller {
    ...
    public function _initialize() {
        ...
        {        
            ...
            if(time() - strtotime($_POST['time']) > 600)
            {    
    ...
    {
        $json_arr = array('status'=>1,'msg'=>'成功!','result'=>date('Y-m-d H:i:s'));
        $json_str = json_encode($json_arr);
        exit($json_str);       
    }
        ...
        */    
        ...
        {                                
                if(empty($this->token))
                    ...
                    if(time() - strtotime($this->user['last_login']) > 3600)   //3600
                        exit(json_encode(array('status'=>-102,'msg'=>'登录超时,请重新登录!!!','result'=>(time() - strtotime($this->user['last_login'])))));
                                        
                    $this->user_id = $this->user['user_id'];                    
                     // 更新最后一次操作时间 如果用户一直操作 则一直不超时
                    M('users')->where("user_id = {$this->user_id}")->save(array('last_login'=>date('Y-m-d H:i:s')));
