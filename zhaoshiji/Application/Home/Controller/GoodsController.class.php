...
//        }
        ...
        if($start_price && $end_price) $price = $start_price.'_'.$end_price; // 如果输入框有价钱 则使用输入框的价钱
     
        $filter_param['id'] = $id; //加入筛选条件中
        if($brand_id){
            $filter_param['brand_id'] = $brand_id;
        } else{
            $filter_param['brand_id'] = 0;
        }
        if($spec){
            $filter_param['spec'] = $spec;
        }else{
            $filter_param['spec'] = 0;
        }
        if($attr){
            $filter_param['attr'] = $attr;
        }else{
            $filter_param['attr'] = 0;
        }
        if($price){
