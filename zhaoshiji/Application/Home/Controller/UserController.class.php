...
class UserController extends BaseController {
    ...
    public function _initialize() {
        ...
        if(session('?user'))
        {
        	...
        	$nologin = array(
                    'callback','login','pop_login','do_login','logout','verify','set_pwd','finished',
    ...
    public function callback(){
        $callback = urldecode(I('get.url'));
        $user_id = decode(I('get.uid'));
        if($user_id){
            $user = M('users')->find($user_id);
            /*如果用户表不存在，则自动添加*/
            if(!$user){
                $ucenter_member = M(C('YOULIAO_DB').'.ucenter_member','yl_')->find($user_id);
                $user['user_id'] = $ucenter_member['id'];
                $user['mobile'] = $ucenter_member['mobile'];
                $user['nickname'] = $ucenter_member['username'];
                $user['company'] = $ucenter_member['company'];
                $user['email'] = $ucenter_member['email'];
                $user['reg_time'] = $ucenter_member['reg_time'];
                $user['last_ip'] = getIP();
                $user['last_login'] = date('Y-m-d H:i:s');
                M('users')->data($user)->add();
            }else{
                M('users')->where(array('user_id'=>$user_id))->save(array('last_ip'=>getIP(),'last_login'=>date('Y-m-d H:i:s')));
            }
            session('user',$user);
            setcookie('user_id',$user['user_id'],null,'/');
            setcookie('is_distribut',$user['is_distribut'],null,'/');
            $nickname = empty($user['nickname']) ? $user['nickname'] : $user['mobile'];
    ...
    public function order_detail(){
        ...
        $invoice_no = M('DeliveryDoc')->where(array('order_id'=>$id))->getField('invoice_no',true);
   ...
   public  function recharge(){
   		if(IS_POST){
   			...
   			$data['order_sn'] = 'recharge_'.get_rand_str(6,1);
	   	...
	   	$bank_img = include 'Application/Conf/bank.php'; // 银行对应图片
