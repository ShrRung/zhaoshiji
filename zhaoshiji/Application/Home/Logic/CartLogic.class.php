...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        {
            $flash_sale = M('flash_sale')->where("id = {$goods['prom_id']} and NOW() > start_time and NOW() < end_time and goods_num > buy_num")->find(); // 限时抢购活动
            if($flash_sale){
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function getCouponMoneyByCode($couponCode,$order_momey)
    {
        ...
        if(date('Y-m-d H:i:s') > $coupon['use_end_time'])
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        if($coupon_id > 0){
        	...
        	$data3['use_time'] = date('Y-m-d H:i:s');
        ...
        $data4['change_time'] = date('Y-m-d H:i:s');
        $data4['desc'] = '下单消费';
   ...
   {
       $group_buy = M('GroupBuy')->where("goods_id = $goods_id and NOW() >= start_time and NOW() <= end_time ")->find(); // 找出这个商品
       if(empty($group_buy))       
            return 0;
