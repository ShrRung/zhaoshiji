...
class GoodsLogic extends RelationModel
{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        foreach ($list_brand as $k => $v) {
            ...
            $list_brand[$k]['href'] = 'list-'.implode('-',$filter_param).'.html';
//            $list_brand[$k]['href'] = urldecode(U("Goods/$action", $filter_param, ''));
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        foreach ($spec_key as $k => $v) {
            if (strpos($old_spec, $spec_item[$v]['spec_id'] . '_') === 0 || strpos($old_spec, 'x' . $spec_item[$v]['spec_id'] . '_'))
                continue;
            ...
            // 筛选参数
            if (!empty($old_spec))
                $filter_param['spec'] = $old_spec . 'x' . $spec_item[$v]['spec_id'] . '_' . $v;
            else
                $filter_param['spec'] = $spec_item[$v]['spec_id'] . '_' . $v;
            $list_spec[$spec_item[$v]['spec_id']]['item'][] = array('key' => $spec_item[$v]['spec_id'], 'val' => $v, 'item' => $spec_item[$v]['item'], 'href' => 'list-'.implode('-',$filter_param).'.html');
//            $list_spec[$spec_item[$v]['spec_id']]['item'][] = array('key' => $spec_item[$v]['spec_id'], 'val' => $v, 'item' => $spec_item[$v]['item'], 'href' => urldecode(U("Goods/$action", $filter_param, '')));
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        ...
        foreach ($goods_attr as $k => $v) {
            // 存在的筛选不再显示
            if (strpos($old_attr, $v['attr_id'] . '_') === 0 || strpos($old_attr, 'x' . $v['attr_id'] . '_'))
                continue;
            ...
            // 筛选参数
            if (!empty($old_attr))
                $filter_param['attr'] = $old_attr . 'x' . $v['attr_id'] . '_' . $v['attr_value'];
            else
                $filter_param['attr'] = $v['attr_id'] . '_' . $v['attr_value'];

//            $list_attr[$v['attr_id']]['attr_value'][] = array('key' => $v['attr_id'], 'val' => $v['attr_value'], 'attr_value' => $v['attr_value'], 'href' => 'list-'.implode('-',$filter_param).'.html');
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        M('GoodsCollect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => date('Y-m-d H:i:s')));
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            $filter_param['price'] = "{$start}_{$end}";
            if ($i == 0)
                $parr[] = array('value' => "{$end}以下", 'href' => 'list-'.implode('-',$filter_param).'.html');
            elseif ($i == ($c - 1))
                $parr[] = array('value' => "{$end}以上", 'href' => 'list-'.implode('-',$filter_param).'.html');
            else
                $parr[] = array('value' => "{$start}-{$end}", 'href' => 'list-'.implode('-',$filter_param).'.html');
        }
        return $parr;
    }
    ...
    function get_filter_menu($filter_param, $action)
    {
        ...
        if (!empty($filter_param['brand_id'])) {
            ...
            $tmp = $filter_param;
            $tmp['brand_id'] = 0; // 当前的参数不再带入
            $brand['href'] = '/list-'.implode('-',$tmp).'.html';
//            $brand['href'] = urldecode(U("Goods/$action", $tmp, ''));
        ...
        if (!empty($filter_param['spec'])) {
            ...
            $spec_group = explode('x', $filter_param['spec']);
            foreach ($spec_group as $k => $v) {
                ...
                $tmp = $spec_group;
                $tmp2 = $filter_param;
                if(count($spec_group)>1){
                    unset($tmp[$k]);
                }else{
                    $tmp[$k] = 0;
                }
                $tmp2['spec'] = implode('x', $tmp); // 当前的参数不再带入

                $spec_menu['href'] = '/list-'.implode('-',$tmp2).'.html';
//                $spec_menu['href'] = urldecode(U("Goods/$action", $tmp2, ''));
        ...
        if (!empty($filter_param['attr'])) {
            ...
            $attr_group = explode('x', $filter_param['attr']);
            foreach ($attr_group as $k => $v) {
                ...
                $tmp2['attr'] = implode('x', $tmp); // 当前的参数不再带入
                $attr_menu['href'] = '/list-'.implode('-',$tmp2).'.html';
//                $attr_menu['href'] = urldecode(U("Goods/$action", $tmp2, ''));
        ...
        if (!empty($filter_param['price'])) {
            ...
            $filter_param['price'] = '0';
            $price_menu['href'] = '/list-'.implode('-',$filter_param).'.html';
