...
class UsersLogic extends RelationModel
{
    ...
    public function app_login($username,$password){
        ...
        if(!$user){
           ...
           $result = array('status'=>-2,'msg'=>'密码错误!'.$password.'=='.$user['password']);
        }elseif($user['is_lock'] == 1){
           $result = array('status'=>-3,'msg'=>'账号异常已被锁定！！！');
        }else{
            //查询用户信息之后, 查询用户的登记昵称
            ...
            M('users')->where("user_id = {$user['user_id']}")->save(array('token'=>$user['token'],'last_login'=>date('Y-m-d H:i:s')));
            $result = array('status'=>1,'msg'=>'登陆成功','result'=>$user);
        }
        return $result;
    }    
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $user = get_user_info($openid,3,$oauth);
			// 会员注册送优惠券
			$coupon = M('coupon')->where("send_end_time > NOW() and ((createnum - send_num) > 0 or createnum = 0) and type = 2")->select();
			foreach ($coupon as $key => $val)
			{
				// 送券            
				M('coupon_list')->add(array('cid'=>$val['id'],'type'=>$val['type'],'uid'=>$user['user_id'],'send_time'=>date('Y-m-d H:i:s')));
    ...
    public function reg($username,$password,$password2){
        ...
        if(!empty($coupon)){
        	foreach ($coupon as $key => $val)
        	{
        		M('coupon_list')->add(array('cid'=>$val['id'],'type'=>$val['type'],'uid'=>$user_id,'send_time'=>date('Y-m-d H:i:s')));
        		M('Coupon')->where("id = {$val['id']}")->setInc('send_num'); // 优惠券领取数量加一
        	}
        }
    ...
    public function get_last_order($user_id){
        $last_order = M('order')->where("user_id = {$user_id}")->order('add_time DESC')->find();
        return $last_order;
    }
...
//        $type = I('get.type',0);
    ...
    public function get_coupon($user_id,$type =0 ){
        ...
        $where = ' AND l.order_id = 0 AND c.use_end_time > NOW()'; // 未使用
        if($type == 1){
            //已使用
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            ...
            left join __PREFIX__order as o on o.order_id = og.order_id  where o.user_id = $user_id and og.is_send = 1  
            $where order by o.add_time desc  LIMIT {$page->firstRow},{$page->listRows}";
        }
    ...
    public function sms_log($mobile,$code,$session_id){
        ...
        //120秒以内不可重复发送
        if($data && (time() - strtotime($data['add_time'])) < $sms_time_out)
            return array('status'=>-1,'msg'=>$sms_time_out.'秒内不允许重复发送');
        $row = M('sms_log')->add(array('mobile'=>$mobile,'code'=>$code,'add_time'=>date('Y-m-d H:i:s'),'session_id'=>$session_id));
        if(!$row)
    ...
    public function sms_code_verify($mobile,$code,$session_id){
        ...
        $sms_time_out = $sms_time_out ? $sms_time_out : 120;
        //验证是否过时
        if((time() - strtotime($data['add_time'])) > $sms_time_out)
    ...
    public function send_validate_code($sender,$type){
    	...
    	if(!empty($send) && strtotime($send['time']) > time() && $send['sender'] == $sender){
    		//在有效期范围内 相同号码不再发送
    	...
    	if($send){
    		...
    		$info['sender'] = $sender;
    		$check['is_check'] = 0;
    		$info['time'] = date('Y-m-d H:i:s',time() + $sms_time_out); //有效验证时间
    ...
    public function check_validate_code($code,$sender){   	
    	...
    	}elseif($check['time'] < date('Y-m-d H:i:s'))
    	{
