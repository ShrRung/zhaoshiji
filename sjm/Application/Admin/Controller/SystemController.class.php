...
class SystemController extends BaseController{
	...
	public function refreshMenu(){
        ...
        public function cleanCache(){              
            ...
            if(IS_POST)
            {                
                 if(in_array('cache',$_POST['clear'])){// 模板缓存
                     delFile('./Application/Runtime/Cache');
                     delFile(C('YOULIAO_ROOT').'Application/Runtime/Cache');
                 }
                if(in_array('data',$_POST['clear'])){// 项目数据
                    delFile('./Application/Runtime/Data');
                    delFile(C('YOULIAO_ROOT').'Application/Runtime/Data');
                }
                if(in_array('logs',$_POST['clear'])){// logs日志
                    delFile('./Application/Runtime/Logs');
                    delFile(C('YOULIAO_ROOT').'Application/Runtime/Logs');
                }
                if(in_array('temp',$_POST['clear'])){// 临时数据
                    delFile('./Application/Runtime/Temp');
                    delFile(C('YOULIAO_ROOT').'Application/Runtime/Temp');
                }
                if(in_array('cacheAll',$_POST['clear'])){// 清除所有
                    delFile('./Application/Runtime');
                    delFile(C('YOULIAO_ROOT').'Application/Runtime');
                }
                 
                // 删除静态文件                
                $html_arr = glob(C('YOULIAO_ROOT')."Application/Runtime/Html/*.html");
     ...
     */        
     public function login_task()
     {
        ...
        /*** 随机清空购物车的垃圾数据*/                     
        $time = date('Y-m-d H:i:s',time() - 3600); // 删除购物车数据  1小时以前的
        M("Cart")->where("user_id = 0 and  add_time < $time")->delete();            
        $today_time = date('Y-m-d H:i:s');
        
        // 发货后满多少天自动收货确认
        $auto_confirm_date = tpCache('shopping.auto_confirm_date');
        $order_id_arr = M('order')->where("order_status = 1 and shipping_status = 1 and  TIMESTAMPDIFF(day,$today_time,shipping_time) >  $auto_confirm_date")->getField('order_id',true);
