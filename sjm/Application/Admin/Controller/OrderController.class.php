...
class OrderController extends BaseController {
    ...
    */
    ...
    public function edit_order(){
       	...
       	if(IS_POST)
        {
            ...
            $o = M('order')->where(array('order_id'=>$order_id))->save($order);
    ...
    public function split_order(){
    	...
    	if(IS_POST){
    		...
    		M('order')->where(array('order_id'=>$order_id))->save($res);
			//################################原单处理结束
    		...
    		foreach($brr as $goods){
    			...
    			$new_order['add_time'] = date('Y-m-d H:i:s');
    			$new_order['order_id'] = uuid();
    ...
    public function delivery_info(){
    	...
    	$order = $orderLogic->getOrderInfo($order_id);
    	$orderGoods = $orderLogic->getOrderGoods($order_id);
		$delivery_record = M('delivery_doc')->where(array('order_id'=>$order_id))->select();
		if($delivery_record){
   ...
   {                
            ...
            $return_goods = M('return_goods')->where(array('order_id'=>$order_id,'goods_id'=>$goods_id))->find();
    ...
    public function order_log(){
    	...
    	$log =  M('order_action');
    	if($gap[0] && $gap[1]){
    		$condition['log_time'] = array('between',"$gap[0],$gap[1]");
    	}
    	$admin_id = I('admin_id');
		if($admin_id >0 ){
    ...
    public function export_order()
    {
		...
		if($timegap){
			$gap = explode('-', $timegap);
			$where .= " AND add_time>".$gap[0]." and add_time< ".$gap[1];
		}
	    ...
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
	    		...
	    		$orderGoods = D('order_goods')->where(array('order_id'=>$val['order_id']))->select();
	    		$strGoods="";
